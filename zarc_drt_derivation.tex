...
\item run a local lambda-perturbation robustness check around the selected $\lambda$,
...
\item run bootstrap sensitivity analysis for DRT uncertainty bands,
\item write temperature-wise Excel outputs (Nyquist/DRT/RCN/KK/peaks/sensitivity/lambda sheets),
\item after all temperatures are processed, generate the RGB multi-temperature peak map and write per-workbook summary sheets.
\end{enumerate}

The code therefore has two complementary layers: a mathematical inversion layer, and a reporting layer that exports all derived quantities for post-processing. That pairing is now standard in modern DRT software, where the inversion method and the exported diagnostics are treated as part of the reproducible method description rather than as an implementation afterthought \cite{schluter2019,maradesa2023,carbone2025}.
...
\node[block, below=of gcv] (drt) {Compute DRT $\gamma(\tau)$ and fitted impedance \\ then evaluate residuals};
\node[block, below=of drt] (lampert) {Lambda-perturbation check around \\ selected $\lambda$};
\node[block, below=of lampert] (peaks) {RBF refinement, hidden-peak detection, \\ Gaussian peak fitting};
...
\node[block, below=of kk] (sens) {Bootstrap DRT sensitivity \\ and uncertainty bands};
\node[block, below=of sens] (exports) {Export per-temperature sheets: \\ NYQ, DRT, RCN, KK, PEAK, SENS, LAMBDA, PFIT};
\node[block, below=of exports] (post) {After loop: generate RGB peak map \\ and workbook summary sheets};
...
\draw[line] (pre) -- (gcv);
\draw[line] (gcv) -- (drt);
\draw[line] (drt) -- (lampert);
\draw[line] (lampert) -- (peaks);
\draw[line] (peaks) -- (rcn);
\draw[line] (rcn) -- (kk);
\draw[line] (kk) -- (sens);
\draw[line] (sens) -- (exports);
\draw[line] (exports) -- (post);
\draw[line] (post) -- (end);
\end{tikzpicture}
\end{center}
...
\item \texttt{calculate\_eis\_from\_rcn\_peaks\_matlab2011}: reconstructs impedance from fitted series R-C-n peak branches.
\end{itemize}
...
\item \texttt{perform\_lambda\_perturbation\_sensitivity\_matlab2011}: checks local residual sensitivity around the selected regularization level.
\item \texttt{perform\_sensitivity\_analysis\_matlab2011}: runs bootstrap uncertainty estimation for the DRT profile.
\item \texttt{generate\_temperature\_rgb\_peak\_map}: creates a multi-temperature RGB visualization from the top fitted peaks.
...
\section{Robustness of the Selected $\lambda$}

After the GCV step has selected $\lambda_{\mathrm{best}}$, the code still checks how stable that choice is under nearby perturbations. The reason is that a low-GCV solution can still be locally fragile: a small change in $\lambda$, or a small random perturbation in the measured spectrum, may move the solution toward a noticeably different DRT if the inverse problem is close to ill-conditioned \cite{tuncer2001,saccoccio2014,maradesa2023,carbone2025}.

The lambda-perturbation study answers a local question: does the residual change smoothly when $\lambda$ is moved above and below the selected value? If the residual curve is flat, then the selected point is not an isolated numerical accident but lies on a stable regularization plateau. The bootstrap analysis answers a different but complementary question: if the spectrum is resampled with realistic noise, do the recovered DRT peaks remain in the same locations and with similar widths and amplitudes? Together, these diagnostics confirm that the GCV-selected $\lambda$ is not only optimal by score, but also robust to small data and parameter perturbations.
...
\item \texttt{\_temperature\_rgb\_peak\_map.png}: RGB map of the top peak components across temperatures,
\item \texttt{drt\_matlab2011\_run.log}: execution log captured with MATLAB diary.
\end{itemize}

The refined peak profile workbook is especially important when individual Gaussian peak components need to be analyzed temperature-by-temperature outside MATLAB. In addition, the script writes summary sheets to the KK, sensitivity, and lambda-perturbation workbooks after all temperatures are completed, so both per-temperature details and run-level aggregates are preserved. Similar exported intermediate products are useful in recent DRT workflows because they make later comparison, cross-validation, and uncertainty checks easier to reproduce \cite{schluter2019,maradesa2023,carbone2025}.
