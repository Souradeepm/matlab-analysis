function run_s2422_peakfit_two_methods()
% run_s2422_peakfit_two_methods
% Peak fitting on S2422 DRTs for two lambda selection methods:
% 1) minimum residual lambda
% 2) L-curve corner lambda
% Hidden peaks are identified using a multiscale + residual strategy.
...
out_xlsx = fullfile(repo_root, 's2422_peakfit_two_methods.xlsx');
...
% ---------------- compute both DRTs ----------------
[gamma_resid, Rinf_resid] = tr_drt_local(freq_vec, Z_exp, lambda_resid);
[gamma_lcurve, Rinf_lcurve] = tr_drt_local(freq_vec, Z_exp, lambda_lcurve);

% ---------------- hidden-peak detection + fitting ----------------
...
fit_curve_resid = build_gaussian_sum(log(tau), fit_resid);
fit_curve_lcurve = build_gaussian_sum(log(tau), fit_lcurve);

fprintf('Residual-min lambda: %.6e | peaks found: %d\n', lambda_resid, numel(fit_resid));
fprintf('L-curve lambda     : %.6e | peaks found: %d\n', lambda_lcurve, numel(fit_lcurve));

% ---------------- plots ----------------
figure('Color', 'w', 'Name', 'S2422 DRT peak fit - residual-min lambda');
semilogx(tau, gamma_resid, 'b-', 'LineWidth', 1.2); hold on;
semilogx(tau, fit_curve_resid, 'k--', 'LineWidth', 1.2);
plot_peak_markers(fit_resid);
set(gca, 'XDir', 'reverse');
xlabel('Relaxation time tau (s)');
ylabel('gamma(tau)');
title(sprintf('Residual-min lambda = %.3e', lambda_resid));
grid on;
legend('DRT', 'Gaussian fit sum', 'Detected peaks', 'Location', 'best');

figure('Color', 'w', 'Name', 'S2422 DRT peak fit - L-curve lambda');
semilogx(tau, gamma_lcurve, 'r-', 'LineWidth', 1.2); hold on;
semilogx(tau, fit_curve_lcurve, 'k--', 'LineWidth', 1.2);
plot_peak_markers(fit_lcurve);
set(gca, 'XDir', 'reverse');
xlabel('Relaxation time tau (s)');
ylabel('gamma(tau)');
...
figure('Color', 'w', 'Name', 'S2422 DRT two-method peak comparison');
semilogx(tau, gamma_resid, 'b-', 'LineWidth', 1.2); hold on;
semilogx(tau, gamma_lcurve, 'r-', 'LineWidth', 1.2);
set(gca, 'XDir', 'reverse');
xlabel('Relaxation time tau (s)');
ylabel('gamma(tau)');
title('Two-method DRT comparison with peak-fit inputs');
grid on;
legend(sprintf('Residual-min (%.3e)', lambda_resid), sprintf('L-curve (%.3e)', lambda_lcurve), 'Location', 'best');

% ---------------- export ----------------
summary = [1, lambda_resid, Rinf_resid, numel(fit_resid), sum([fit_resid.R]); ...
           2, lambda_lcurve, Rinf_lcurve, numel(fit_lcurve), sum([fit_lcurve.R])];

xlswrite(out_xlsx, {'method_code','lambda','R_inf','peak_count','total_peak_R'}, 'summary', 'A1');
xlswrite(out_xlsx, summary, 'summary', 'A2');

xlswrite(out_xlsx, {'tau_s','gamma_residual_method','gamma_lcurve_method','fit_sum_residual_method','fit_sum_lcurve_method'}, 'drt_curves', 'A1');
xlswrite(out_xlsx, [tau, gamma_resid, gamma_lcurve, fit_curve_resid, fit_curve_lcurve], 'drt_curves', 'A2');

write_peak_table(out_xlsx, 'peaks_residual_method', fit_resid);
write_peak_table(out_xlsx, 'peaks_lcurve_method', fit_lcurve);

fprintf('Saved peak-fit workbook: %s\n', out_xlsx);
end

function write_peak_table(file_name, sheet_name, fit_struct)
headers = {'peak_id','tau_peak_s','amplitude','sigma_logtau','R_est','C_est','n_est','is_hidden'};
xlswrite(file_name, headers, sheet_name, 'A1');
...
M = zeros(n, 8);
for i = 1:n
    M(i,1) = fit_struct(i).peak_id;
    M(i,2) = fit_struct(i).tau;
    M(i,3) = fit_struct(i).amplitude;
    M(i,4) = fit_struct(i).sigma;
    M(i,5) = fit_struct(i).R;
    M(i,6) = fit_struct(i).C;
    M(i,7) = fit_struct(i).n;
    M(i,8) = fit_struct(i).is_hidden;
end
xlswrite(file_name, M, sheet_name, 'A2');
end

function plot_peak_markers(fit_struct)
if isempty(fit_struct)
    return;
end
...
% Multiscale strong-peak detection + residual shoulder detection for hidden peaks.
y = y(:);
n = numel(y);
...
% Hidden-peak channel: high-pass residual against broad baseline.
...
% Merge near indices to avoid duplicates from multiple scales.
cand = sort(cand(:));
merged = cand(1);
for i = 2:numel(cand)
...
% Mark hidden peaks if they appear in hidden list and are weak in base channel.
...
% safety: remove boundary artifacts
...
fit_struct = repmat(struct('peak_id',0,'tau',0,'amplitude',0,'sigma',0,'R',0,'C',0,'n',0,'is_hidden',0), numel(peak_idx), 1);
...
% sort by tau descending for readability
